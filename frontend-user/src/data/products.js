...
// 可参与方案对照的产品 id（智慧仓储 / 运输管理 / 配送调度）
export const COMPARABLE_IDS = ['wms', 'tms', 'dms']
...
// 可对照产品列表（仅智慧仓储 / 运输管理 / 配送调度）
export const comparableProducts = products.filter(p => p.comparable)
...
export const parseCompareIds = (raw) => {
  ...
  const ids = String(raw).split(',')
    .map(s => s.trim())
    .filter(s => COMPARABLE_IDS.includes(s))
  return [...new Set(ids)].slice(0, 2)
}

// 生成带对照结果的咨询链接
export const buildContactQuery = (compareIds) => {
