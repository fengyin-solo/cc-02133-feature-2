...
// 参与方案对照的产品（智慧仓储 / 运输管理 / 配送调度）
export const comparableIds = ['wms', 'tms', 'dms']

// 对照选择上限
export const COMPARE_MAX = 2

/**
 * 解析对照参数（支持字符串或数组），过滤无效项：
 * 仅保留可对照产品、去重、最多 COMPARE_MAX 项。
 * 返回空数组表示无效选择。
 */
export const parseCompareIds = (raw) => {
  ...
  list.forEach((item) => {
    ...
    if (comparableIds.includes(id) && !ids.includes(id)) {
      ids.push(id)
    }
  })
